-- Return privacy-aware raw statistics records and favourites so a friend's
-- statistics page can use the same cards and drill-downs as the owner view.
...
begin
  ...
  if can_statistics then
    ...
    select coalesce(jsonb_object_agg(favourite.category, favourite.value), '{}'::jsonb)
    into favourite_rows
    from public.user_favourites as favourite
    where favourite.user_id = friend_user_id;
  end if;
  ...
  return jsonb_build_object(
    ...
    'statistics_flights', statistics_rows,
    'statistics_summary', statistics_summary,
    'favourites', favourite_rows
  );
end;
$$;
